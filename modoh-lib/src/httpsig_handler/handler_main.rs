...
use crate::{
  ...
  error::*,
  globals::Globals,
  hyper_body::{BoxBody, IncomingOr, full},
  ...
  C: Send + Sync + Connect + Clone + 'static,
{
  ...
  pub(crate) async fn get_hmac_signing_key_by_domain(&self, domain: &str) -> Option<IndexMap<Generation, SessionKeyNonce>> {
    ...
    let one_key_id_for_each_generation = available_key_ids
      .iter()
      .map(|(genx, key_ids)| (genx, key_ids.first().unwrap()));
    let futs = one_key_id_for_each_generation.map(|(genx, key_id)| async move {
      ...
      session_key_with_random_nonce.ok().map(|v| (*genx, v))
    });
