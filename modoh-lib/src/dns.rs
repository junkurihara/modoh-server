// porting of https://github.com/DNSCrypt/doh-server/blob/master/src/libdoh/src/dns.rs
...
fn traverse_rrs<F: FnMut(usize) -> Result<(), Error>>(
  ...
  for _ in 0..rrcount {
    ...
    offset += 10;
    ensure!(packet_len - offset >= rdlen, "Record length would exceed packet length");
    offset += rdlen;
  }
  Ok(offset)
}
...
pub fn set_edns_max_payload_size(packet: &mut Vec<u8>, max_payload_size: u16) -> Result<(), Error> {
  ...
  offset += 4;
  let (ancount, nscount, arcount) = (ancount(packet), nscount(packet), arcount(packet));
  offset = traverse_rrs(packet, offset, ancount as usize + nscount as usize, |_offset| Ok(()))?;
